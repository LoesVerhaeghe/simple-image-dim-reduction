...
combined_df_10min = combined_df.resample('10T').first()
...
df_effluent['time'] = (df_effluent.index - combined_df_10min.index[0]).total_seconds() / (3600 * 24)
df_effluent.set_index('time', inplace=True)
df_effluent.to_csv('data/TSS_effluent.csv', sep='\t')
...
combined_df_10min.to_csv('data/input_settler_model.csv', sep='\t')
...
smoothed_df=combined_df_10min.copy()

smoothed_df = clean_and_smooth(smoothed_df, 'TSS_RAS', window_size=100, method='mean')
smoothed_df = clean_and_smooth(smoothed_df, 'MLSS_basin5', window_size=100, method='mean')
...
smoothed_df.to_csv('data/input_settler_model_smoothed.csv', sep='\t')